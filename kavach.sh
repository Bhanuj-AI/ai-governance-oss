...
usage() {
  cat <<'EOF'
Usage: ./kavach.sh <command> [service ...]

Commands:
  up       Start Keycloak and the local Kavach stack (default).
  ...
  down     Stop the Kavach Compose stack and preserve local data.
...
restart_services() {
  ...
  if (( ${#running[@]} == 0 )); then
    echo "No Kavach Compose services are currently running." >&2
    return 1
  fi
  ...
  if (( $# > 0 )); then
    for service in "$@"; do
      if ! contains "$service" "${running[@]}"; then
        echo "'$service' is not a running Kavach Compose service." >&2
...
case "$command" in
  up)
    ...
    echo
    echo "Kavach OSS development stack is running."
    echo "Studio: http://localhost:3000"
    echo "API:    http://localhost:8000"
    ;;
  build)
    prepare_runtime
    "${COMPOSE[@]}" build kavach-platform kavach-studio
    ;;
  restart)
    restart_services "$@"
    ;;
