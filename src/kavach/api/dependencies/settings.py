...
def get_api_settings() -> ApiSettings:
    ...
    environment = os.getenv("KAVACH_ENV", "local").strip().lower()
    identity_provider = os.getenv("KAVACH_AUTH_MODE", "development").strip().lower()
    ...
    if (
        ...
        raise ValueError(
            ...
            "Set KAVACH_ALLOW_DEVELOPMENT_IDENTITY_IN_PRODUCTION=true only for an explicit exception."
        )
    return ApiSettings(
        host={"localhost": "127.0.0.1"}.get(
            os.getenv("KAVACH_API_HOST", "127.0.0.1"),
            os.getenv("KAVACH_API_HOST", "127.0.0.1"),
        ),
        environment=environment,
        port=int(os.getenv("KAVACH_API_PORT", "8000")),
        log_level=os.getenv("KAVACH_API_LOG_LEVEL", "info"),
        cors_allow_origins=_split_csv_env(
            "KAVACH_CORS_ALLOW_ORIGINS",
            "http://localhost:3000,http://127.0.0.1:3000",
        ),
        auto_seed_demo_data=_auto_seed_demo_data_enabled(),
        tenancy_enabled=os.getenv("KAVACH_TENANCY_ENABLED", "true").strip().lower()
        in {"1", "true", "yes", "on"},
        identity_provider=identity_provider,
        auth_mode=os.getenv("KAVACH_AUTH_MODE", "development").strip().lower(),
        oidc_issuer=os.getenv("KAVACH_OIDC_ISSUER"),
        oidc_jwks_refresh_seconds=int(
...
def _auto_seed_demo_data_enabled() -> bool:
    explicit = os.getenv("KAVACH_AUTO_SEED_DEMO_DATA")
    if explicit is not None:
        return explicit.strip().lower() in {"1", "true", "yes", "on"}

    environment = os.getenv("KAVACH_ENV", "local").strip().lower()
    host = os.getenv("KAVACH_API_HOST", "127.0.0.1").strip().lower()
    return (
