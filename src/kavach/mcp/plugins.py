...
from kavach.mcp.mappers import MCPResponseMapper
...
MCP_TOOL_PLUGIN_ENTRY_POINT_GROUP = "kavach.mcp.plugins"
...
class MCPToolPluginMetadata:
    ...
    def __post_init__(self) -> None:
        name, version, required_kavach_version = (
            self.name,
            self.version,
            self.required_kavach_version,
        )
        if not name.strip() or not version.strip() or not required_kavach_version.strip():
            raise ValueError(
                "MCP tool plugin metadata requires name, version, and Kavach version range."
            )
...
class MCPToolPluginRegistry:
    ...
    def __init__(self, *, kavach_version: str = __version__) -> None:
        self._kavach_version = kavach_version
        self._plugins: dict[str, MCPToolPlugin] = {}

    def discover(self) -> None:
        """Load plugins published through the ``kavach.mcp.plugins`` group."""
        ...
        for plugin in self._plugins.values():
            ...
            try:
                compatible = Version(self._kavach_version) in SpecifierSet(
                    metadata.required_kavach_version
                )
            except Exception as exc:
                raise MCPToolPluginError(
                    f"MCP tool plugin '{metadata.name}' has an invalid Kavach version range."
                ) from exc
            if not compatible:
                raise MCPToolPluginError(
                    f"MCP tool plugin '{metadata.name}' requires Kavach "
                    f"{metadata.required_kavach_version}, but running version is "
                    f"{self._kavach_version}."
                )
            try:
