...
class S3DatasetObjectStore:
    ...
    def from_environment(cls) -> S3DatasetObjectStore:
        ...
        endpoint_url = _optional_environment("KAVACH_DATASET_S3_ENDPOINT_URL")
        region_name = os.getenv("KAVACH_DATASET_S3_REGION", "us-east-1")
        access_key = _optional_environment("KAVACH_DATASET_S3_ACCESS_KEY_ID")
        secret_key = _optional_environment("KAVACH_DATASET_S3_SECRET_ACCESS_KEY")
        force_path_style = _boolean_environment(
            "KAVACH_DATASET_S3_FORCE_PATH_STYLE",
...
def dataset_object_store_from_environment() -> DatasetObjectStore | None:
    ...
    backend = os.getenv("KAVACH_DATASET_OBJECT_STORE_BACKEND", "none").strip().lower()
    if backend in {"", "none"}:
        return None
    if backend == "s3":
        return S3DatasetObjectStore.from_environment()
    raise ValueError(
        "KAVACH_DATASET_OBJECT_STORE_BACKEND must be 'none' or 's3'."
    )
