...
from kavach.hooks import FailurePolicy, HookHandler, HookRegistry
...
from kavach.plugins.contracts import CONTRACT_VERSION, MiddlewareDefinition
...
class ContributionRegistry:
    ...
    def emit(self, name: str, attributes: dict[str, object]) -> None:
        from kavach.plugins.contracts import TelemetryEvent
        event = TelemetryEvent(name, dict(attributes))
        for exporter in self.items("telemetry_exporters"):
            try:
                exporter(event)
            except Exception:
                logging.getLogger("kavach.extensions").exception("plugin_exporter_failed")
...
class PluginRegistry:
    ...
    The registry is created by the Kavach application factory. It discovers
    ...
    def __init__(
        ...
        *,
        ...
        kavach_version: str = __version__,
    ) -> None:
        """Create an isolated runtime registry for one Kavach process.

        Args:
            supported_capabilities: OSS allow-list that plugin metadata is
                checked against before any plugin callback executes.
            kavach_version: Version used for compatibility checks. Tests and
        ...
        self._kavach_version = kavach_version
        self._records: dict[str, _PluginRecord] = {}

    def register(self, plugin: KavachPlugin) -> None:
    ...
    def discover(self, group: str = "kavach.plugins") -> None:
    ...
    def stop(self) -> None:
        ...
        for record in reversed(tuple(self._records.values())):
            ...
            try:
                ...
                logging.getLogger("kavach.extensions").exception(
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        return {
            "kavach_version": self._kavach_version,
            "plugins": [
                {
    ...
    def _validate(self, record: _PluginRecord, context: PluginContext) -> None:
        try:
            compatible = Version(self._kavach_version) in SpecifierSet(
                record.metadata.required_kavach_version
            )
        except Exception as exc:
            raise ExtensionError(
        ...
        if not compatible:
            ...
            raise ExtensionError(
                f"Plugin '{record.metadata.name}' requires Kavach "
                f"{record.metadata.required_kavach_version}, running {self._kavach_version}."
            )
...
def _install_settings(items: tuple[object, ...]) -> None:
    if not items:
        return
    from kavach.settings_control.registry import register_extension_definitions
    register_extension_definitions(items)

def _install_permissions(items: tuple[object, ...]) -> None:
    if items:
        from kavach.tenancy.permissions import register_extension_permissions
        register_extension_permissions(items)

def _install_job_handlers(items: tuple[object, ...]) -> None:
    if items:
        from kavach.services.job_executor import register_extension_handlers
