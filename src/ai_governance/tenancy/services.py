...
def provision_mcp_service_account(
    ...
    *,
    ...
    actor_id = actor_id or os.getenv("AI_GOVERNANCE_MCP_ACTOR_ID")
    _provision_service_account(
        repository,
        organization_id=organization_id,
        actor_id=actor_id,
        display_name="AI Governance Control Plane MCP service account",
        role_prefix="mcp",
...
def provision_walkthrough_service_account(
    ...
    *,
    ...
    actor_id = actor_id or os.getenv("AI_GOVERNANCE_WALKTHROUGH_ACTOR_ID")
    _provision_service_account(
        repository,
        organization_id=organization_id,
        actor_id=actor_id,
        display_name="AI Governance Control Plane walkthrough service account",
...
def _resolve_bootstrap_administrator(
    ...
    *,
    ...
    Development mode
    ----------------
    Uses ``AI_GOVERNANCE_DEVELOPMENT_ACTOR_ID`` (fallback: ``local-admin``) and
    ``AI_GOVERNANCE_DEVELOPMENT_ACTOR_NAME`` (fallback: ``Local Administrator``).

    Keycloak mode
    -------------
    Requires ``AI_GOVERNANCE_BOOTSTRAP_ADMIN_SUB`` (the immutable Keycloak user
    ...
    if auth_mode == "development":
        ...
        administrator_name = os.getenv("AI_GOVERNANCE_BOOTSTRAP_ADMIN_NAME")
    else:
