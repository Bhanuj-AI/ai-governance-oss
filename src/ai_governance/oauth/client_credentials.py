...
def client_credentials_from_environment(
    ...
    ``AI_GOVERNANCE_OAUTH_*`` is the supported workload-neutral interface. The MCP
    ...
    generic = {
        "token_url": values.get("AI_GOVERNANCE_OAUTH_TOKEN_URL"),
        "client_id": values.get("AI_GOVERNANCE_OAUTH_CLIENT_ID"),
        "client_secret": values.get("AI_GOVERNANCE_OAUTH_CLIENT_SECRET"),
    }
    if any(generic.values()):
        missing = [name for name, value in generic.items() if not value]
        if missing:
            raise OAuthClientCredentialsError(
                "Incomplete AI_GOVERNANCE_OAUTH client-credentials configuration: "
                f"missing {', '.join(missing)}."
            )
        return OAuthClientCredentials(**generic)  # type: ignore[arg-type]

    legacy = {
        "token_url": values.get("AI_GOVERNANCE_MCP_TOKEN_URL"),
        "client_id": values.get("AI_GOVERNANCE_MCP_CLIENT_ID"),
        "client_secret": values.get("AI_GOVERNANCE_MCP_CLIENT_SECRET"),
    }
