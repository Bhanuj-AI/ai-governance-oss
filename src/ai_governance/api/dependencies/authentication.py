...
logger = logging.getLogger("ai_governance.api.authentication")
...
def get_authentication_service(
    ...
    if not oidc_issuer:
        raise ValueError(
            "AI_GOVERNANCE_OIDC_ISSUER is required when AI_GOVERNANCE_AUTH_MODE=keycloak"
        )
...
def get_authenticated_principal(
    ...
    Development mode
    ----------------
    When ``AI_GOVERNANCE_AUTH_MODE=development`` the existing behaviour is preserved:
    the actor identity comes from ``X-AI-Governance-Actor-Id`` header or the
    ``AI_GOVERNANCE_DEVELOPMENT_ACTOR_ID`` environment variable. No JWT validation
    occurs.

    Keycloak mode
    -------------
    When ``AI_GOVERNANCE_AUTH_MODE=keycloak`` the ``Authorization: Bearer <token>``
...
def _development_principal() -> AuthenticatedPrincipal:
    ...
    identity comes from ``X-AI-Governance-Actor-Id`` or
    ``AI_GOVERNANCE_DEVELOPMENT_ACTOR_ID``.
    """
    import os
