...
- `src/ai_governance/mcp/server.py:create_server` assembles the tool registry and the
  REST client used by handlers.
- Optional MCP tool packages are discovered through the `ai_governance.mcp.plugins`
...
- `MCPExecutionAuditRecord` in `src/ai_governance/mcp/audit.py` is the durable audit
