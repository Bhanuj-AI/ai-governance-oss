...
from ai_governance.mcp.dto import ToolCallResult, ToolDescription
from ai_governance.mcp.exceptions import map_exception
from ai_governance.mcp.handlers.control_plane_tools import register_control_plane_tools
from ai_governance.mcp.handlers.evaluation_tools import register_evaluation_tools
from ai_governance.mcp.handlers.experiment_tools import register_experiment_tools
from ai_governance.mcp.handlers.governance_tools import register_governance_tools
from ai_governance.mcp.handlers.job_tools import register_job_tools
from ai_governance.mcp.handlers.mcp_audit_tools import register_mcp_audit_tools
from ai_governance.mcp.handlers.ontology_graph_tools import (
    register_ontology_graph_tools,
)
from ai_governance.mcp.handlers.provider_tools import register_provider_tools
from ai_governance.mcp.handlers.registry_tools import register_registry_tools
...
logger = logging.getLogger("ai_governance.mcp")
...
def get_mcp_settings() -> MCPSettings:
    transport = os.getenv("AI_GOVERNANCE_MCP_TRANSPORT", "stdio").strip().lower()
    if transport not in {"stdio", "streamable-http"}:
        raise ValueError("AI_GOVERNANCE_MCP_TRANSPORT must be 'stdio' or 'streamable-http'.")
    http_path = os.getenv("AI_GOVERNANCE_MCP_HTTP_PATH", "/mcp").strip() or "/mcp"
    if not http_path.startswith("/"):
        raise ValueError("AI_GOVERNANCE_MCP_HTTP_PATH must start with '/'.")
    public_url = os.getenv("AI_GOVERNANCE_MCP_PUBLIC_URL", "http://localhost:8002").strip()
    if not public_url:
        raise ValueError("AI_GOVERNANCE_MCP_PUBLIC_URL must not be empty.")
    protected_resource_url = f"{public_url.rstrip('/')}{http_path}"
    return MCPSettings(
        api_url=os.getenv("AI_GOVERNANCE_API_URL", "http://127.0.0.1:8000"),
        api_timeout=float(os.getenv("AI_GOVERNANCE_API_TIMEOUT", "10")),
        api_retries=int(os.getenv("AI_GOVERNANCE_API_RETRIES", "0")),
        log_level=os.getenv("LOG_LEVEL", "INFO"),
        audit_repository=os.getenv("AI_GOVERNANCE_MCP_AUDIT_REPOSITORY", "sqlite")
        .strip()
        .lower(),
        ...
        audit_postgres_dsn=os.getenv("AI_GOVERNANCE_MCP_AUDIT_POSTGRES_DSN"),
        transport=transport,
        http_host=os.getenv("AI_GOVERNANCE_MCP_HTTP_HOST", "127.0.0.1").strip(),
        http_port=int(os.getenv("AI_GOVERNANCE_MCP_HTTP_PORT", "8002")),
        http_path=http_path,
        http_allowed_hosts=_split_csv_env(
            "AI_GOVERNANCE_MCP_HTTP_ALLOWED_HOSTS", "localhost,127.0.0.1"
        ),
        http_allowed_origins=_split_csv_env("AI_GOVERNANCE_MCP_HTTP_ALLOWED_ORIGINS", ""),
        http_stateless=_bool_env("AI_GOVERNANCE_MCP_HTTP_STATELESS", True),
        http_json_response=_bool_env("AI_GOVERNANCE_MCP_HTTP_JSON_RESPONSE", True),
        protected_resource_url=protected_resource_url,
    )


class AIGovernanceMCPServer:
    """
    Stateless MCP server facade for read-first AI Governance Control Plane tool calls.
    """
...
def create_server(
    ...
    logging.getLogger("ai_governance.mcp").setLevel(settings.log_level.upper())
...
def _development_principal() -> AuthenticatedPrincipal:
    actor_id = os.getenv("AI_GOVERNANCE_DEVELOPMENT_ACTOR_ID")
...
def _api_headers() -> dict[str, str]:
    ...
    token = os.getenv("AI_GOVERNANCE_API_TOKEN", "").strip()
    return {"Authorization": f"Bearer {token}"} if token else {}


def _keycloak_token_provider() -> Any:
    """Return a cached client-credentials token provider for long-lived MCP."""
    if os.getenv("AI_GOVERNANCE_API_TOKEN", "").strip():
        return None
    token_url = os.getenv("AI_GOVERNANCE_MCP_TOKEN_URL", "").strip()
    client_id = os.getenv("AI_GOVERNANCE_MCP_CLIENT_ID", "").strip()
    client_secret = os.getenv("AI_GOVERNANCE_MCP_CLIENT_SECRET", "").strip()
