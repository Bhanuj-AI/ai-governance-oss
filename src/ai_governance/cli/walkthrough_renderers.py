...
class PlainRenderer:
    ...
    def render_manifest(self, manifest: WalkthroughManifest) -> str:
        from ai_governance.cli.walkthrough import format_walkthrough
...
class RichRenderer:
    ...
    def emit(self, event: WalkthroughLifecycleEvent) -> None:
        if event.name == "walkthrough_started":
            ...
            title = event.data.get("title", "AI Governance Control Plane Walkthrough")
            subtitle = (
            ...
            self.console.print(
                Panel(
                    f"[bold cyan]AI_GOVERNANCE GOVERNED REPLAY QUEST[/]\n[dim]{subtitle}[/]",
                    title=title,
                    border_style="cyan",
                    padding=(1, 4),
                )
    ...
    def offer_manifest_cleanup(self) -> None:
        ...
        from ai_governance.cli.walkthrough import cleanup_walkthrough_manifests
