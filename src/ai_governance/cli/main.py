"""Command-line entry point for AI Governance Control Plane guided walkthroughs."""
...
def main() -> None:
    ...
    if arguments.command != "walkthrough" or arguments.walkthrough not in {
        ...
        parser.error("Choose a supported walkthrough. Run 'ai-governance walkthrough list'.")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ai-governance",
        description="Guided AI Governance Control Plane product walkthroughs that use only public REST APIs.",
    )
    parser.add_argument(
        "--version",
        action="version",
    ...
    replay.add_argument(
        "--api-url", default=os.getenv("AI_GOVERNANCE_API_URL", "http://localhost:8000")
    )
    replay.add_argument(
        "--studio-url", default=os.getenv("AI_GOVERNANCE_STUDIO_URL", "http://localhost:3000")
    )
    replay.add_argument(
        "--organization-id",
        default=os.getenv("AI_GOVERNANCE_ORGANIZATION_ID", "org_default"),
    )
    replay.add_argument(
        "--project-id", default=os.getenv("AI_GOVERNANCE_PROJECT_ID", "project_default")
    )
    replay.add_argument(
        "--token",
        help="Explicit bearer-token override. By default the CLI uses AI_GOVERNANCE_OAUTH_* credentials.",
    )
    ...
    replay.add_argument(
        "--manifest", help="Write the run manifest to this path instead of .ai-governance/."
    )
    replay.add_argument(
        "--output-json",
        "--json",
...
def _resolve_walkthrough_token(explicit_token: str | None) -> str | None:
    ...
    if os.getenv("AI_GOVERNANCE_AUTH_MODE", "").lower() == "development":
        return None
    try:
        ...
        raise WalkthroughError(
            ...
            "Complete AI_GOVERNANCE_OAUTH_* client credentials or pass --token explicitly. "
            f"Details: {exc}"
        ) from exc
