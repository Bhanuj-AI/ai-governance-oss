"""Public lifecycle contract implemented by AI Governance Control Plane plugins."""

from __future__ import annotations

from typing import TYPE_CHECKING, Protocol

from ai_governance.plugins.metadata import PluginMetadata

if TYPE_CHECKING:
    from ai_governance.plugins.registry import PluginContext


class AIGovernancePlugin(Protocol):
    """Stable lifecycle protocol implemented by every AI Governance Control Plane extension.

    AI Governance Control Plane calls the methods in this order: :attr:`metadata`, ``validate()``,
    ``register()``, ``start()``, and finally ``stop()`` during application
    shutdown. Registration must only use the supplied :class:`PluginContext`;
    plugins must not reach into AI Governance Control Plane services, repositories, or application
