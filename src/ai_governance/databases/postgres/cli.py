"""Initialize PostgreSQL schemas before AI Governance Control Plane application processes start."""
...
def migration_dsns_from_environment() -> tuple[str, ...]:
    ...
    value = os.getenv("AI_GOVERNANCE_POSTGRES_MIGRATION_DSNS", "")
    dsns = tuple(item.strip() for item in value.split("|") if item.strip())
    if not dsns:
        raise ValueError("AI_GOVERNANCE_POSTGRES_MIGRATION_DSNS must contain at least one DSN")
    return dsns
