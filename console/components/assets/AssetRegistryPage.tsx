...
function RegistryTable({ kind }: { kind: AssetKind }) {
  ...
  const rows = kind === "prompts" ? selectLogicalAssets(prompts.data ?? [], (item) => item.name).map((item) => ({ id: item.prompt_id, name: item.name, version: item.version, status: item.status, details: `${item.variables.length} variables`, updated: item.created_at }))
    : kind === "models" ? selectLogicalAssets(models.data ?? [], (item) => `${item.provider}:${item.model_name}`).map((item) => ({ id: item.model_id, name: item.model_name, version: item.version, status: item.status, details: `${item.provider} · ${item.context_window.toLocaleString()} context`, updated: item.created_at }))
      : kind === "datasets" ? selectLogicalAssets(datasets.data ?? [], (item) => item.name).map((item) => ({ id: item.dataset_id, name: item.name, version: item.version, status: item.status, details: `${item.record_count.toLocaleString()} records · ${item.schema_version}`, updated: item.created_at }))
        : (providers.data ?? []).map((item) => ({ id: item.name, name: item.display_name, version: item.version, status: "ACTIVE", details: `${item.capabilities.supported_metrics.length} metrics`, updated: "" }));
  ...
  return <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-5 px-6 py-5">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="rounded-md bg-primary/10 p-2 text-primary"><Icon className="h-5 w-5" /></div><div><div className="flex items-center gap-2"><h1 className="text-2xl font-semibold">{info.title}</h1><AssetProvenanceBadge kind={kind} /></div><p className="mt-1 text-sm text-muted-foreground">{info.description}</p></div></div>{kind === "datasets" ? <DatasetUploadForm openInitially={onboardingRegistration} /> : null}</div>
    {kind === "providers" ? <ProviderInstallations providers={providers.data ?? []} installations={installations.data ?? []} loading={loading || installations.isLoading} error={failed || installations.isError} openInitially={openProviderInstallation} /> : <Card><CardContent className="p-0">{loading ? <div className="p-8 text-sm text-muted-foreground">Loading registry…</div> : failed ? <div className="p-8 text-sm text-destructive">Unable to load this registry.</div> : rows.length === 0 ? <div className="p-10 text-center text-sm text-muted-foreground">No assets have been registered.</div> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-muted-foreground"><th className="p-4">Name</th><th>Current Version</th><th>Status</th><th>Registry Details</th><th className="pr-4 text-right">Updated</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b last:border-0 hover:bg-accent/40"><td className="p-4"><Link href={`/assets/${kind}/${encodeURIComponent(row.id)}`} className="font-medium text-primary underline decoration-primary/45 underline-offset-4 transition-colors hover:text-primary/75 hover:decoration-primary">{row.name}</Link></td><td>{row.version}</td><td><StatusBadge status={row.status} /></td><td className="text-muted-foreground">{row.details}</td><td className="pr-4 text-right text-muted-foreground">{row.updated ? new Date(row.updated).toLocaleDateString() : "—"}</td></tr>)}</tbody></table></div>}</CardContent></Card>}
  </div>;
}
