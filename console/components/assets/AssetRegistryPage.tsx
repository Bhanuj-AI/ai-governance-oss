...
function RegistryTable({ kind }: { kind: AssetKind }) {
  ...
  return <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-5 px-6 py-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3">{kind === "prompts" || kind === "models" ? <div className="flex gap-1 rounded-md border p-1">{(["ALL", "MANAGED", "OBSERVED"] as const).map((value) => <Button key={value} size="sm" variant={provenance === value ? "secondary" : "ghost"} onClick={() => setProvenance(value)}>{value === "ALL" ? "All" : value === "MANAGED" ? kind === "models" ? "Registered" : "Managed" : "Observed"}</Button>)}</div> : <span />}{kind === "datasets" ? <DatasetUploadForm openInitially={onboardingRegistration} /> : kind === "prompts" || kind === "models" ? <ManagedAssetForm kind={kind} /> : null}</div>
