...
function RuntimeDefaultsForm({
  ...
  return <section className="grid gap-3 rounded-md border p-3 sm:col-span-2">
    <div><div className="font-medium text-sm">Runtime defaults</div><p className="mt-1 text-xs text-muted-foreground">Only controls supported by this model are shown. Leave a value blank to use the provider default.</p></div>
    <div className="grid gap-3 sm:grid-cols-3">{parameters.map((parameter) => <label key={parameter.name} className="grid gap-1 text-sm">{parameter.name.replaceAll("_", " ")}<Input name={`runtime-default-${parameter.name}`} type="number" step={parameter.value_type === "integer" ? "1" : "any"} min={parameter.minimum ?? undefined} max={parameter.maximum ?? undefined} placeholder={parameter.default == null ? "Provider default" : String(parameter.default)} value={values[parameter.name] ?? ""} onChange={(event) => { const next = { ...values }; if (event.target.value === "") delete next[parameter.name]; else next[parameter.name] = Number(event.target.value); onChange(next); }} /></label>)}</div>
  </section>;
}
...
export function ManagedAssetForm({ kind }: { kind: "prompts" | "models" }) {
  ...
  return <form onSubmit={submit} className="grid w-full max-w-3xl gap-3 rounded-md border bg-card p-4 sm:grid-cols-2">
    ...
    {kind === "models" ? <label className="grid gap-1 text-sm">Model catalog connection<select value={catalogConnectionId} onChange={(event) => { setCatalogConnectionId(event.target.value); setProviderModelId(""); }} className="h-9 rounded-md border bg-background px-3 text-sm"><option value="">Select an active {selectedProvider || "provider"} connection</option>{(runtimeConnections.data ?? []).filter((connection) => connection.enabled && connection.provider === selectedProvider).map((connection) => <option key={connection.runtime_connection_id} value={connection.runtime_connection_id}>{connection.display_name}</option>)}</select><span className="text-xs text-muted-foreground">The catalog is read using this connection; credentials stay private.</span></label> : null}
...
export function ModelVersionForm({ model }: { model: ModelAsset }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setError(null); create.mutate({ version: String(form.get("version")).trim(), provider_model_id: providerModelId.trim(), context_window: Number(form.get("context_window")), parameters: runtimeDefaults }); }} className="grid gap-3 rounded-md border bg-card p-4">
    <div><div className="font-medium">Create managed model version</div><p className="mt-1 text-xs text-muted-foreground">{model.provider} · {model.model_name}. This creates an immutable DRAFT version.</p></div><label className="grid gap-1 text-sm">Version<Input name="version" required placeholder="v2" /></label><label className="grid gap-1 text-sm">Model catalog connection<select value={catalogConnectionId} onChange={(event) => { setCatalogConnectionId(event.target.value); setProviderModelId(""); setRuntimeDefaults({}); }} className="h-9 rounded-md border bg-background px-3 text-sm"><option value="">Select an active {model.provider} connection</option>{(runtimeConnections.data ?? []).filter((connection) => connection.enabled && connection.provider.toLowerCase() === model.provider.toLowerCase()).map((connection) => <option key={connection.runtime_connection_id} value={connection.runtime_connection_id}>{connection.display_name}</option>)}</select></label><label className="grid gap-1 text-sm">Provider model ID<select name="provider_model_id" required value={providerModelId} disabled={!catalogConnectionId || discoveredModels.isLoading} onChange={(event) => { setProviderModelId(event.target.value); setRuntimeDefaults({}); }} className="h-9 rounded-md border bg-background px-3 text-sm disabled:opacity-70"><option value="">{discoveredModels.isLoading ? "Discovering models…" : "Select a provider model"}</option>{(discoveredModels.data ?? []).map((item) => <option key={item.provider_model_id} value={item.provider_model_id}>{item.provider_model_id}</option>)}</select>{discoveredModels.isError ? <span className="text-xs text-destructive">Unable to discover models for this connection.</span> : <span className="text-xs text-muted-foreground">Exact identifier sent to the provider.</span>}</label><label className="grid gap-1 text-sm">Context window<Input name="context_window" type="number" min="1" required defaultValue={model.context_window} /></label>{versionCapabilities ? <RuntimeDefaultsForm capabilities={versionCapabilities} values={runtimeDefaults} onChange={setRuntimeDefaults} /> : <p className="rounded-md border p-3 text-sm text-muted-foreground">Loading runtime controls for this provider model ID…</p>}{error ? <p className="text-sm text-destructive">{error}</p> : null}<div className="flex gap-2"><Button type="submit" disabled={create.isPending || !providerModelId || (requiresModelDiscovery && !catalogConnectionId) || (capabilityRefreshRequired && resolvedCapabilities.isLoading)}>{create.isPending ? "Creating…" : "Create Version"}</Button><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={create.isPending}>Cancel</Button></div>
  </form>;
}
