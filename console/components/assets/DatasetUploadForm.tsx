...
export function DatasetUploadForm({ openInitially = false }: { openInitially?: boolean }) {
  ...
  const upload = useMutation({
    ...
    onError: (reason) =>
      setError(reason instanceof AIGovernanceApiError ? reason.message : "Dataset upload failed."),
  });
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-md border bg-card p-4 sm:grid-cols-2">
    <div className="sm:col-span-2"><div className="text-sm font-medium">Register Dataset Version</div><p className="mt-1 text-xs text-muted-foreground">Upload UTF-8 CSV or JSONL. AI Governance Control Plane stores immutable bytes in the configured S3-compatible store and registers a draft version.</p></div>
