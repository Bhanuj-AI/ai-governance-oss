...
function configuration(kind: AssetKind, asset: PromptAsset | ModelAsset | DatasetAsset | ProviderAsset, promptTemplate?: string | null) {
  ...
  if (kind === "models") { const model = asset as ModelAsset; const parameterRows: [string, string][] = Object.entries(model.parameters).map(([key, value]) => [humanize(key), renderValue(value)]); const capabilityRows: [string, string][] = model.runtime_capabilities.parameters.map((parameter) => [humanize(parameter.name), parameter.supported ? "Supported" : "Provider default only"]); return <MetadataList rows={[["Source system", model.source_system ?? "Not recorded"], ["Source reference", model.source_reference ?? "Not recorded"], ["Provider model ID", model.provider_model_id ?? model.model_name], ["Context window", model.context_window.toLocaleString()], ["Runtime contract", `${model.runtime_capabilities.invocation_contract} · ${model.runtime_capabilities.verification.toLowerCase()}`], ...capabilityRows, ...parameterRows]} />; }
