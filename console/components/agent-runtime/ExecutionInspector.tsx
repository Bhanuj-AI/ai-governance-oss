...
function EventTimelineItem({ event }: { event: AgentExecutionEventDto }) {
  ...
  const isWorkflowStep = event.event_type === "WORKFLOW_STEP";
  const eventLabel = isWorkflowStep
    ? `${event.step_name ?? "Workflow Step"} · Step ${humanize(event.lifecycle ?? "")}`
    : humanize(event.event_type);
  const workflowFacts: Array<[string, string | null]> = isWorkflowStep
    ? [
        ["Step ID", event.step_id],
        ["Parent step", event.parent_step_id],
...
function formatDuration(start: string, end: string | null) { if (!end) return "Running"; const milliseconds = new Date(end).getTime() - new Date(start).getTime(); if (!Number.isFinite(milliseconds) || milliseconds < 0) return "—"; if (milliseconds < 1_000) return `${milliseconds}ms`; const seconds = Math.floor(milliseconds / 1_000); return seconds < 60 ? `${seconds}s` : `${Math.floor(seconds / 60)}m ${seconds % 60}s`; }
